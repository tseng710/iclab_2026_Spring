...
//   Release version : V19.1 (1-Seed-3-Scenarios & Ultimate PPA Edition)
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//############################################################################
...
integer base_seed = `SEED_NUMBER;
integer current_seed;
integer dump_seed; 
integer dummy_rand; // 🌟 用來消耗一次亂數，保持平行宇宙的同步
integer test_round; 
integer NUM_TEST_SEEDS  = 20; // 🌟 測試 20 個不同的 SEED
integer NUM_TEST_ROUNDS = 60; // 🌟 20 SEEDs × 3 種狀況 = 60 趟

// 🌟 PPA 統計陣列與變數
integer current_pat_num;
integer history_seed     [0:99];
integer history_scenario [0:99];
integer history_pat_num  [0:99];
integer history_latency  [0:99];
integer total_marathon_latency = 0;
integer scenario_mode; 
...
initial begin
    ...
    total_marathon_latency = 0;

    for (test_round = 0; test_round < NUM_TEST_ROUNDS; test_round = test_round + 1) begin
        
        // 🌟 每 3 個 Round 才換一個 SEED，這樣同一個 SEED 就會經歷 3 種狀況！
        current_seed = base_seed + (test_round / 3);
        scenario_mode = test_round % 3;
        
        dump_seed = $urandom(current_seed); 
        dummy_rand = $urandom_range(0, 9999999);
        
        // 🌟 決定這一回合的目標步數
        if (scenario_mode == 0)      current_pat_num = 4096; // Mode 0: 剛好跑滿
        else if (scenario_mode == 1) current_pat_num = 1000 + (dummy_rand % 3000); // Mode 1: 提早結束
        else                         current_pat_num = 4500 + (dummy_rand % 3500); // Mode 2: 死迴圈

        history_seed[test_round]     = current_seed;
        history_scenario[test_round] = scenario_mode;
        history_pat_num[test_round]  = current_pat_num;

        if(`MAX_WAIT_READY_CYCLE == 2000) begin
            $display("==========================================================================");
            $display("  [INFO] STARTING MARATHON RUN #%0d with SEED: %0d", test_round + 1, current_seed);
            $display("  [INFO] PATTERN Mode: FUNC | Target Steps (PAT_NUM): %0d", current_pat_num);
            $display("==========================================================================");
        end
        if(`MAX_WAIT_READY_CYCLE == 100000) begin
            $display("==========================================================================");
            $display("  [INFO] STARTING MARATHON RUN #%0d with SEED: %0d", test_round + 1, current_seed);
            $display("  [INFO] PATTERN Mode: PERF | Target Steps (PAT_NUM): %0d", current_pat_num);
            $display("==========================================================================");
        end
        ...
        // 🌟 統計數據
        history_latency[test_round] = latency;
        total_marathon_latency = total_marathon_latency + latency;

        $display("\033[0;34m[INFO] MARATHON RUN #%0d (SEED %0d, Mode %0d) PASSED!\033[m", test_round + 1, current_seed, scenario_mode);
        $display("\033[0;32m       Execution Steps: %5d, Latency Cycle: %5d\033[m", TOTAL_STEPS, latency);
    ...
    // 🌟 PPA 漂亮總結表
    $display("\n=====================================================================");
    $display("  \033[1;33m[SUCCESS] Congratulations! All %0d MARATHON RUNS PASSED!\033[m ", NUM_TEST_ROUNDS);
    $display("=====================================================================");
    $display("  Run # |  SEED  | Scenario | PAT_NUM (Steps) | Latency (Cycles)");
    $display("---------------------------------------------------------------------");
    for (integer r = 0; r < NUM_TEST_ROUNDS; r = r + 1) begin
        $display("   %2d   |  %4d  |  Mode %0d  |      %5d      |      %6d", r + 1, history_seed[r], history_scenario[r], history_pat_num[r], history_latency[r]);
    end
    $display("---------------------------------------------------------------------");
    $display("  \033[1;36m[PPA STATS] Total Latency   : %0d Cycles\033[m", total_marathon_latency);
    $display("  \033[1;36m[PPA STATS] Average Latency : %0d Cycles\033[m", total_marathon_latency / NUM_TEST_ROUNDS);
    $display("=====================================================================\n");
    $finish;
end
...
task generate_and_simulate_task;
    ...
    begin
        ...
        $fdisplay(fd_trace, "==========================================================================");
        $fdisplay(fd_trace, "                        MARATHON TRACE (SEED: %0d)", current_seed);
        $fdisplay(fd_trace, "==========================================================================");

        // 🌟 跑到動態目標步數為止！
        while (step < current_pat_num && pc1 <= 4095 && pc2 <= 4095) begin
            ...
            if (!valid_inst1[pc1]) begin
                if (pc1 < 8) begin
                    ...
                    else begin op1 = 3'b100; rs1 = 3'd0; rt1 = pc1[2:0]; imm1_ext = $urandom_range(0, 31); inst1_word = {op1, rs1, rt1, imm1_ext[6:0]}; temp_imm1 = imm1_ext; end
                end else begin
                    ...
                    if (pc1 == 4095) begin
                        ...
                        if (op1 == 4 || op1 == 5) begin
                            ...
                            for (k = 0; k < 3; k = k + 1) begin
                                if (($signed(sim_r1[k]) + 100 >= 0) && ($signed(sim_r1[k]) - 100 <= 4095)) begin 
                            ...
                            min_imm = -100; max_imm = 100;
                            if ((0 - $signed(sim_r1[rs1])) > min_imm) min_imm = (0 - $signed(sim_r1[rs1]));
                            if ((4095 - $signed(sim_r1[rs1])) < max_imm) max_imm = (4095 - $signed(sim_r1[rs1]));
                            
                            temp_imm1 = min_imm + ($unsigned($urandom) % (max_imm - min_imm + 1));
                            if (temp_imm1 > 63) temp_imm1 = 63;
                            if (temp_imm1 < -64) temp_imm1 = -64;
                            ...
                            jump_offset1 = $urandom_range(0, 255) - 128;
            ...
            if (!valid_inst2[pc2]) begin
                if (pc2 < 8) begin
                    ...
                    else begin op2 = 3'b100; rs2 = 3'd0; rt2 = pc2[2:0]; imm2_ext = $urandom_range(0, 31); inst2_word = {op2, rs2, rt2, imm2_ext[6:0]}; temp_imm2 = imm2_ext; end
                end else begin
                    ...
                    if (pc2 == 4095) begin
                        ...
                        if (op2 == 4 || op2 == 5) begin
                            valid_rs_count = 0;
                            for (k = 0; k < 3; k = k + 1) begin
                                if (($signed(sim_r2[k]) + 100 >= 0) && ($signed(sim_r2[k]) - 100 <= 4095)) begin
                                    valid_rs_list[valid_rs_count] = k; valid_rs_count = valid_rs_count + 1;
                                end
                            end
                            if (valid_rs_count == 0) begin valid_rs_list[0] = 0; valid_rs_count = 1; end 
                            ...
                            min_imm = -100; max_imm = 100;
                            if ((0 - $signed(sim_r2[rs2])) > min_imm) min_imm = (0 - $signed(sim_r2[rs2]));
                            if ((4095 - $signed(sim_r2[rs2])) < max_imm) max_imm = (4095 - $signed(sim_r2[rs2]));
                            
                            temp_imm2 = min_imm + ($unsigned($urandom) % (max_imm - min_imm + 1));
                            if (temp_imm2 > 63) temp_imm2 = 63;
                            if (temp_imm2 < -64) temp_imm2 = -64;

                            inject_hazard = $urandom_range(0, 99);
                            if ((op1 == 4 || op1 == 5) && (op2 == 4 || op2 == 5)) begin
                                if (inject_hazard > 50) begin
                                    expected_addr1 = 4096 + ($signed(sim_r1[rs1]) + temp_imm1) * 2;
                                    for (k = 0; k < 3; k = k + 1) begin
                            ...
                            jump_offset2 = $urandom_range(0, 255) - 128;
        ...
        for (i = 0; i < 4096; i = i + 1) begin
            if(i == 0) begin $fdisplay(fd_inst1, "//MARATHON RUN, SEED = %0d", current_seed); $fdisplay(fd_inst2, "//MARATHON RUN, SEED = %0d", current_seed); end
