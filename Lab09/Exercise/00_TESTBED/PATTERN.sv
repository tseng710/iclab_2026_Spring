...
function automatic void generate_inputs();
    ...
    if (pat < 42) begin
        ...
        target_shop_id = (safe_idx != -1) ? safe_idx : 0;
        in_mth = 12; in_day = 31; cur_in_type = Fruit_Cake;
        cur_in_mode = Party_Pack;
    end 
    else if (pat < 192) begin
        cur_in_act = Restock;
        in_mth = 12; in_day = 31;
        rand_flour = 2047; rand_butter = 2047; rand_milk = 2047; rand_sugar = 2047;
        rand_fruit = 2047; 
        safe_idx = 0;
        ...
        cur_in_act = Hire_Staff;
        in_mth = 12; in_day = 31; rand_hire_cnt = 30;
        safe_idx = 0;
        ...
        if (safe_idx != -1) begin
            rand_flour = 2047;
            rand_butter = 0; rand_milk = 0; rand_sugar = 0; rand_fruit = 0; 
            target_shop_id = safe_idx;
        end 
        else begin
            rand_flour = 2047;
            rand_butter = 2047; rand_milk = 2047; rand_sugar = 2047; rand_fruit = 2047;
        end
    end 
...
task automatic compute_golden();
    ...
    if (cur_in_act == Make_and_Sell) begin
        ...
        if (expected_warn != Date_Warn) begin cur_shop.M = in_mth; cur_shop.D = in_day; end
        ...
        if (is_early) expected_warn = Date_Warn;
            ...
            if (expected_warn == Restock_Warn || expected_warn == No_Warn) begin
                t_f += ad_f;
                t_b += ad_b; t_m += ad_m; t_s += ad_s; t_fr += ad_fr;
                t_bal -= tot_cst;
            end
        end
        if (expected_warn != Date_Warn) begin cur_shop.M = in_mth; cur_shop.D = in_day; end
...
task automatic wait_out_valid();
    ...
    @(negedge clk);
    // Lab09 隱藏檢查：確認 out_valid 只有一個 cycle
...
// Lab09 隱藏檢查：SVA 檢查替換成 Procedural Blocks (抓 AXI 重疊)
initial begin
    forever begin
        @(posedge clk);
        if (inf.rst_n) begin
            if (inf.AR_VALID && inf.AW_VALID) begin
                $display("Wrong Answer");
                $finish;
            end
        end
...
initial begin
    forever begin
        ...
        if (inf.W_VALID && inf.W_READY) begin
            ...
            if (write_addr_fifo.size() > 0) act_aw_addr = write_addr_fifo.pop_front();
            else if (inf.AW_VALID && inf.AW_READY) act_aw_addr = inf.AW_ADDR;
            else begin
                $display("Wrong Answer");
                $finish;
            end
...
initial begin
    ...
    base_p_tbl[0]=120; base_p_tbl[1]=100; base_p_tbl[2]=400; base_p_tbl[3]=180; base_p_tbl[4]=250; base_p_tbl[5]=200;
    base_p_tbl[6]=280; base_p_tbl[7]=160;
    ...
    req_m_tbl[0]=0;    req_m_tbl[1]=50;
    req_m_tbl[2]=40;   req_m_tbl[3]=150;  req_m_tbl[4]=0;    req_m_tbl[5]=80;   req_m_tbl[6]=0;    req_m_tbl[7]=30;
    ...
    req_fr_tbl[0]=0;   req_fr_tbl[1]=0;   req_fr_tbl[2]=100; req_fr_tbl[3]=20;  req_fr_tbl[4]=0;   req_fr_tbl[5]=40;
    req_fr_tbl[6]=0;   req_fr_tbl[7]=10;
    ...
    if (inf.out_valid !== 0 || inf.warn_msg !== 0 || inf.complete !== 0 ||
        ...
        inf.W_DATA !== 0 || inf.B_READY !== 0) begin
        $display("Wrong Answer"); 
        $finish;
    end
