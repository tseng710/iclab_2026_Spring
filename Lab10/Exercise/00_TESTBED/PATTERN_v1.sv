...
function automatic void generate_inputs();
    ...
    // Coverage targeting: Action transitions
    ...
    // Coverage targeting: Complete signals
    else begin
        in_act = Make_and_Sell;
        in_M = golden_shop[in_shop_id].M + $urandom_range(0, 1);
        if (in_M > 12) in_M = 12;
    ...
    // Sequential mapping for combinations coverage
...
task automatic drive_action(Action act);
    ...
    @(negedge clk); inf.sel_action_valid = 0;
    inf.D = 'bx;
endtask
...
task automatic drive_restock(int amt);
    ...
    @(negedge clk); inf.restock_valid = 0;
    inf.D = 'bx;
endtask
...
task automatic wait_out_valid();
    ...
    while(inf.out_valid !== 1) begin
        ...
        if (lat > 1000) begin
            ...
            $display("Pattern: %0d | Action: %s | Shop ID: %0d", pat, in_act.name(), in_shop_id);
            $display("================================================================\n");
            $finish;
        end
        @(negedge clk);
    ...
    if (inf.complete !== golden_complete || inf.warn_msg !== golden_warn_msg) begin
        ...
        $display("Pattern: %0d | Action: %s | Shop ID: %0d", pat, in_act.name(), in_shop_id);
        $display("----------------------------------------------------------------");
        $display("Expected -> Complete: %b, Warn_Msg: %s", golden_complete, golden_warn_msg.name());
        $display("Actual   -> Complete: %b, Warn_Msg: %s", inf.complete, inf.warn_msg.name());
        $display("================================================================\n");
        $finish;
    end
    ...
    if (golden_warn_msg === Date_Warn && in_act !== Check_Valid_Date) begin
        if (word0_written || word1_written) begin
            $display("\n================================================================");
            $display("ERROR: Unnecessary DRAM write occurred during Date_Warn.");
            $display("Pattern: %0d | Action: %s | Shop ID: %0d", pat, in_act.name(), in_shop_id);
            $display("================================================================\n");
            $finish;
        end
        ...
        if (!word0_written) begin
            $display("\n================================================================");
            $display("ERROR: Missing DRAM Write for Word 0.");
            $display("Pattern: %0d | Action: %s | Shop ID: %0d", pat, in_act.name(), in_shop_id);
            $display("================================================================\n");
            $finish;
        end
        if ((golden_complete === 1 && in_act !== Check_Valid_Date) ||
            ...
            if (!word1_written) begin
                $display("\n================================================================");
                $display("ERROR: Missing DRAM Write for Word 1.");
                $display("Pattern: %0d | Action: %s | Shop ID: %0d", pat, in_act.name(), in_shop_id);
                $display("================================================================\n");
                $finish;
            end
...
// AW_ADDR validation (Decoupled from W_DATA)
initial begin
    forever begin
        @(posedge clk);
        if (inf.AW_VALID && inf.AW_READY) begin
            int expected_addr;
            if (aw_handshake_cnt == 0) begin
                expected_addr = 17'h10000 + (in_shop_id * 16);
                aw_handshake_cnt = 1;
            end else if (aw_handshake_cnt == 1) begin
                expected_addr = 17'h10000 + (in_shop_id * 16) + 8;
                aw_handshake_cnt = 2;
            end

            if (inf.AW_ADDR !== expected_addr) begin
                $display("\n================================================================");
                $display("ERROR: AXI AW_ADDR mismatch.");
                $display("Pattern: %0d | Action: %s", pat, in_act.name());
                $display("Expected AW_ADDR: %h", expected_addr);
                $display("Actual AW_ADDR:   %h", inf.AW_ADDR);
                $display("================================================================\n");
                $finish;
            end
        end
...
// W_DATA validation (Decoupled from AW_ADDR)
initial begin
    forever begin
        @(posedge clk);
        if (inf.W_VALID && inf.W_READY) begin
            ...
            if (w_handshake_cnt == 0) begin
                gold_data = {golden_shop[in_shop_id].flour, golden_shop[in_shop_id].butter, golden_shop[in_shop_id].M, golden_shop[in_shop_id].milk, golden_shop[in_shop_id].sugar, golden_shop[in_shop_id].D};
                word0_written = 1;
                w_handshake_cnt = 1;
            end else if (w_handshake_cnt == 1) begin
                gold_data = {golden_shop[in_shop_id].fruit, golden_shop[in_shop_id].sales, golden_shop[in_shop_id].staff, golden_shop[in_shop_id].balance, golden_shop[in_shop_id].level};
                word1_written = 1;
                w_handshake_cnt = 2;
            end else begin
                $display("\n================================================================");
                $display("ERROR: Too many W_VALID handshakes detected in a single action.");
                $display("Pattern: %0d | Action: %s", pat, in_act.name());
                $display("================================================================\n");
                $finish;
            end
            ...
            if (inf.W_DATA !== gold_data) begin
                $display("\n================================================================");
                $display("ERROR: AXI W_DATA mismatch for Word %0d.", w_handshake_cnt-1);
                $display("Pattern: %0d | Action: %s", pat, in_act.name());
                $display("----------------------------------------------------------------");
                $display("Expected W_DATA: %h", gold_data);
                $display("Actual W_DATA:   %h", inf.W_DATA);
                $display("================================================================\n");
                $finish;
            end
        end
