...
//   File Name   : PATTERN.v
//   Module Name : PATTERN
//   Description : Marathon Validation Version (Multiple Testcases with Reset)
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//############################################################################
...
//---------------------------------------------------------------------
//   TA DEMO FLAG & MARATHON SETTINGS
//---------------------------------------------------------------------
parameter flag_gen = 0; 
integer base_seed = 233252;
parameter real DESIGN_AREA = 141587;
...
`ifdef FUNC
`define PAT_NUM 10              // Number of independent testcases (DRAM sets) to run
`define TRACE_LENGTH 952        // Number of instructions per testcase
`define MAX_WAIT_READY_CYCLE 2000
`endif

`ifdef PERF
`define PAT_NUM 3               // For PERF, run fewer cases but longer traces
`define TRACE_LENGTH 4500
`define MAX_WAIT_READY_CYCLE 100000
`endif

parameter MAX_TRACE_LENGTH = 5000; // Array size boundary
...
`protect
//---------------------------------------------------------------------
//   REGISTER ARRAY MAPPING (RTL -> PATTERN)
//---------------------------------------------------------------------
...
reg signed [15:0] golden_reg_core1 [0:MAX_TRACE_LENGTH-1][0:7];
reg signed [15:0] golden_reg_core2 [0:MAX_TRACE_LENGTH-1][0:7];
reg [15:0] inst_1_mem [0:4095];
reg [15:0] inst_2_mem [0:4095];

reg [15:0] init_dram_data   [0:4095];
reg [15:0] golden_dram_data [0:4095];
reg [15:0] dram_snapshots [1:(MAX_TRACE_LENGTH/50 + 1)][0:4095];
reg [15:0] trace_inst_1 [0:MAX_TRACE_LENGTH-1];
reg [15:0] trace_inst_2 [0:MAX_TRACE_LENGTH-1];

// Track absolute PC values to precisely determine Halt Condition (Last Address)
reg [12:0] pc_trace_1 [0:MAX_TRACE_LENGTH-1];
reg [12:0] pc_trace_2 [0:MAX_TRACE_LENGTH-1];

real final_perf_score;
real avg_latency;

// Dynamic Halt Condition Signals
...
// ================================================================
// Outstanding Inflight AXI Tracker (Deadlock & Flush Prevention)
// ================================================================
...
// AXI Synchronous State Tracking (False-fail prevention)
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (inflight_r_timer_1 > 2000 || inflight_r_timer_2 > 2000 || inflight_r_timer_d > 2000 || inflight_w_timer_d > 2000) begin
            $display("\n%s========================================================%s", `C_RED, `C_RST);
            $display("%s   [VIOLATION] AXI Transaction Deadlock!%s", `C_RED, `C_RST);
...
// ================================================================
// AXI 300 Cycles Ready Timeout Check (Spec Rule #20)
// ================================================================
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (b_timeout_1 > 300 || r_timeout_1 > 300 || 
            ...
            $display("\n%s========================================================%s", `C_RED, `C_RST);
            $display("%s   [VIOLATION] AXI Latency Violation! (Spec Rule #20)%s", `C_RED, `C_RST);
...
// ================================================================
// AXI Protocol Strict Checker
// ================================================================
always @(posedge clk) begin
    if (rst_n) begin
        // Posedge Sync Check: Valid must not drop if Ready was low in the previous cycle
        if (prev_arvalid_1_track && !prev_arready_1_track && !arvalid_s_inf_inst_1) begin
            $display("\n%s[AXI CRITICAL VIOLATION] Core 1 ARVALID dropped before ARREADY!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        if (prev_arvalid_2_track && !prev_arready_2_track && !arvalid_s_inf_inst_2) begin
            $display("\n%s[AXI CRITICAL VIOLATION] Core 2 ARVALID dropped before ARREADY!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        if (prev_arvalid_d_track && !prev_arready_d_track && !arvalid_s_inf_data) begin
            $display("\n%s[AXI CRITICAL VIOLATION] Data ARVALID dropped before ARREADY!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        if (prev_awvalid_d_track && !prev_awready_d_track && !awvalid_s_inf_data) begin
            $display("\n%s[AXI CRITICAL VIOLATION] Data AWVALID dropped before AWREADY!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        if (prev_wvalid_d_track && !prev_wready_d_track && !wvalid_s_inf_data) begin
            $display("\n%s[AXI CRITICAL VIOLATION] Data WVALID dropped before WREADY!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        
        // Write Protection Check
        if (awvalid_s_inf_inst_1 || wvalid_s_inf_inst_1 || bvalid_s_inf_inst_1) begin
            $display("\n%s[VIOLATION] Instruction DRAM 1 is READ ONLY! Write or BVALID detected!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        if (awvalid_s_inf_inst_2 || wvalid_s_inf_inst_2 || bvalid_s_inf_inst_2) begin
            $display("\n%s[VIOLATION] Instruction DRAM 2 is READ ONLY! Write or BVALID detected!%s", `C_RED, `C_RST);
            display_wrong; $finish;
        end
        
        // Spec Properties Check
        if (arvalid_s_inf_inst_1 && arready_s_inf_inst_1) begin
            if (arsize_s_inf_inst_1 !== 3'b001) begin $display("\n%s[VIOLATION] arsize_s_inf_inst_1 MUST be 3'b001 (2 Bytes)%s", `C_RED, `C_RST); display_wrong; $finish; end
            if (arburst_s_inf_inst_1 !== 2'b01) begin $display("\n%s[VIOLATION] arburst_s_inf_inst_1 MUST be 2'b01 (INCR)%s", `C_RED, `C_RST); display_wrong; $finish; end
        end
        if (arvalid_s_inf_inst_2 && arready_s_inf_inst_2) begin
            if (arsize_s_inf_inst_2 !== 3'b001) begin $display("\n%s[VIOLATION] arsize_s_inf_inst_2 MUST be 3'b001 (2 Bytes)%s", `C_RED, `C_RST); display_wrong; $finish; end
            if (arburst_s_inf_inst_2 !== 2'b01) begin $display("\n%s[VIOLATION] arburst_s_inf_inst_2 MUST be 2'b01 (INCR)%s", `C_RED, `C_RST); display_wrong; $finish; end
        end
        if (arvalid_s_inf_data && arready_s_inf_data) begin
            if (arsize_s_inf_data !== 3'b001) begin $display("\n%s[VIOLATION] arsize_s_inf_data MUST be 3'b001 (2 Bytes)%s", `C_RED, `C_RST); display_wrong; $finish; end
            if (arburst_s_inf_data !== 2'b01) begin $display("\n%s[VIOLATION] arburst_s_inf_data MUST be 2'b01 (INCR)%s", `C_RED, `C_RST); display_wrong; $finish; end
        end
        if (awvalid_s_inf_data && awready_s_inf_data) begin
            if (awsize_s_inf_data !== 3'b001) begin $display("\n%s[VIOLATION] awsize_s_inf_data MUST be 3'b001 (2 Bytes)%s", `C_RED, `C_RST); display_wrong; $finish; end
            if (awburst_s_inf_data !== 2'b01) begin $display("\n%s[VIOLATION] awburst_s_inf_data MUST be 2'b01 (INCR)%s", `C_RED, `C_RST); display_wrong; $finish; end
        end
    end
end

// Consecutive Stall Watchdog
...
initial begin
    ...
    if (flag_gen) begin
        ...
        generate_instructions_and_data(`TRACE_LENGTH);
        $display("\n%s[INFO] PATTERN GENERATION MODE: .dat files generated successfully. (flag_gen = 1)%s\n", `C_GRN, `C_RST);
        $finish;
    end

    $display("\n%s========================================================%s", `C_PUR, `C_RST);
    $display("%s   🚀 Starting Marathon Validation (Running %0d Testcases)%s", `C_PUR, `PAT_NUM, `C_RST);
    $display("%s========================================================%s\n", `C_PUR, `C_RST);

    // ================================================================
    // MARATHON LOOP: Run Multiple DRAM Sets with Hardware Reset
    // ================================================================
    for (pat_idx = 0; pat_idx < `PAT_NUM; pat_idx = pat_idx + 1) begin
        // 動態改變 Seed，確保每筆測資完全不同
        current_seed = base_seed + pat_idx * 17;
        ...
        // 1. 產生新的 DRAM 資料，並寫入 File 與 Model
        generate_instructions_and_data(`TRACE_LENGTH);
        ...
        grand_total_latency = grand_total_latency + total_latency;
        $display("%s   [INFO] Passed Testcase %0d/%0d (IC: %0d | Cycles: %0d)%s", `C_GRN, pat_idx + 1, `PAT_NUM, `TRACE_LENGTH, total_latency, `C_RST);
    end
    
    avg_latency = grand_total_latency / `PAT_NUM;
    final_perf_score = avg_latency * `CYCLE_TIME * (DESIGN_AREA * DESIGN_AREA);
    
    $display("\n%s========================================================%s", `C_GRN, `C_RST);
    $display("%s   [🎉 Marathon Validation Complete 🎉]%s", `C_CYN, `C_RST);
    $display("%s   Passed all %0d independent testcases!%s", `C_WHT, `PAT_NUM, `C_RST);
    $display("%s   ----------------------------------------------------- %s", `C_CYN, `C_RST);
    $display("%s   Avg Cycles (C)    : %.0f %s", `C_WHT, avg_latency, `C_RST);
    $display("%s   Clock Period (T)  : %.2f ns %s", `C_WHT, `CYCLE_TIME, `C_RST);
    $display("%s   Design Area  (A)  : %.2f %s", `C_WHT, DESIGN_AREA, `C_RST);
    $display("%s   🚀 Est. Perf Score: %e %s", `C_YLW, final_perf_score, `C_RST);
    $display("%s========================================================%s\n", `C_GRN, `C_RST);
    display_success;
    $finish;
end
...
task reset_task;
    integer reg_idx;
    begin
        // 1. 每次重置前，強制凍結 Clock，從物理層面消滅撞 Clock edge 的可能
        force clk = 0;
        ...
        // 2. 在 rst_n = 0 期間給予 3 個 Cycles 等待 Buffer Tree
        #(3 * `CYCLE_TIME);
        
        // --- 以下保留原本的檢查邏輯 ---
        if (stall_1 !== 1 || stall_2 !== 1) begin
            $display("\n%s[VIOLATION] stall_1 & stall_2 MUST be HIGH after reset!%s", `C_RED, `C_RST);
            display_wrong;
            $finish;
        end
        ...
        if (awvalid_s_inf_inst_1 !== 0 || wvalid_s_inf_inst_1 !== 0 || arvalid_s_inf_inst_1 !== 0 ||
            ...
            $display("\n%s========================================================%s", `C_RED, `C_RST);
            $display("%s   [VIOLATION] AXI output signals (Valid DFFs) are NOT zero after reset!%s", `C_RED, `C_RST);
            $display("%s========================================================%s\n", `C_RED, `C_RST);
            display_wrong;
            $finish;
        end
        ...
        for (reg_idx = 0; reg_idx < 8; reg_idx = reg_idx + 1) begin
            if (`REG_C1(reg_idx) !== 16'd0 || `REG_C2(reg_idx) !== 16'd0) begin
                $display("\n%s[VIOLATION] All registers must be ZERO after reset!%s", `C_RED, `C_RST);
                display_wrong;
                $finish;
            end
        end
        ...
        // 4. Release 後給予 3 Cycles 的 Grace Period 緩衝
        #(3 * `CYCLE_TIME);

        // 5. 確保 Reset 完全穩定後，解除凍結，讓 Clock 重新開始跑
        release clk;
    end
endtask

task generate_instructions_and_data;
    input integer prog_length;
    ...
    begin
        ...
        for (j = 0; j < 4096; j = j + 1) begin
            inst_1_mem[j] = 16'd0;
            inst_2_mem[j] = 16'd0;
            safe_random_val = ($random(current_seed)) % 100;
        ...
        loop_target = (prog_length > 4000) ? ((prog_length - 3900) / 4) : 0;
        
        if (loop_target > 0) begin
            init_dram_data[0] = loop_target;
            golden_dram_data[0] = loop_target;
        end
        ...
        for (ic = 0; ic < prog_length; ic = ic + 1) begin
            steps_left = (prog_length - 1) - ic;
            is_new_1 = 0; is_new_2 = 0;

            if (!gen_flag1[pc1]) begin
                is_new_1 = 1;
                ...
                if (loop_target > 0 && pc1 < 6) begin
                    ...
                    op1 = $unsigned($random(current_seed)) % 8;
                    rs1 = $random(current_seed) & 3'b111; rt1 = $random(current_seed) & 3'b111; rd1 = $random(current_seed) & 3'b111; rl1 = $random(current_seed) & 3'b111;
                    func1 = $random(current_seed) & 3'b111;
                    temp_imm1 = ($random(current_seed) & 7'h7F) - 64;
                    ...
                    if (pc1 == 4095) begin
                        ...
                        if (op1 == 3'b111) begin
                            ...
                            if (min_imm > max_imm) op1 = 3'b000; 
                                ...
                                jump_offset1 = min_imm + ($unsigned($random(current_seed)) % (max_imm - min_imm + 1));
                                jmp_addr1 = (pc1 + jump_offset1) * 2;
                            end
                        end
                        ...
                        if (op1 == 3'b110) begin
                            ...
                            if (pc1 + 1 + max_imm > max_next_pc) max_imm = (max_next_pc > pc1 + 1) ? (max_next_pc - pc1 - 1) : 0;
                            ...
                            if (min_imm > max_imm) begin 
                                ...
                                else temp_imm1 = min_imm + ($unsigned($random(current_seed)) % (max_imm - min_imm + 1));
                            end
                        end
                    end
                    if (op1 == 3'b001) while (rd1 == rl1) rl1 = $random(current_seed) & 3'b111;
                    if (op1 == 3'b100 || op1 == 3'b101) begin 
                        ...
                        if (valid_rs_count == 0) op1 = 3'b010;
                            ...
                            rs1 = valid_rs_list[$unsigned($random(current_seed)) % valid_rs_count];
                            rs1_val = sim_r1[rs1];
            ...
            if (!gen_flag2[pc2]) begin
                ...
                if (loop_target > 0 && pc2 < 6) begin
                    ...
                    op2 = $unsigned($random(current_seed)) % 8;
                    rs2 = $random(current_seed) & 3'b111; rt2 = $random(current_seed) & 3'b111; rd2 = $random(current_seed) & 3'b111; rl2 = $random(current_seed) & 3'b111;
                    func2 = $random(current_seed) & 3'b111;
                    temp_imm2 = ($random(current_seed) & 7'h7F) - 64;
                    ...
                    if (pc2 == 4095) begin
                        ...
                        if (pc2 == 4095) begin
                            ...
                            if (op2 == 3'b111) begin // JUMP
                                ...
                                if (min_imm > max_imm) op2 = 3'b000;
                                    ...
                                    jump_offset2 = min_imm + ($unsigned($random(current_seed)) % (max_imm - min_imm + 1));
                                    jmp_addr2 = (pc2 + jump_offset2) * 2;
                                end
                            ...
                            if (op2 == 3'b110) begin // BEQ
                                ...
                                temp_imm2 = min_imm;
                                    ...
                                    else temp_imm2 = min_imm + ($unsigned($random(current_seed)) % (max_imm - min_imm + 1));
                                end
                            end
                        end
                    end 
                        ...
                        if (op2 == 3'b110) begin
                            ...
                            if (min_imm > max_imm) op2 = 3'b000;
                                ...
                                temp_imm2 = min_imm + ($unsigned($random(current_seed)) % (max_imm - min_imm + 1));
                            end
                        end
                    end
                    if (op2 == 3'b001) while (rd2 == rl2) rl2 = $random(current_seed) & 3'b111;
                    if (op2 == 3'b100 || op2 == 3'b101) begin 
                        ...
                        if (valid_rs_count == 0) op2 = 3'b010;
                            ...
                            rs2 = valid_rs_list[$unsigned($random(current_seed)) % valid_rs_count];
                            rs2_val = sim_r2[rs2];
            ...
            if (is_new_1 && is_new_2) begin
                if ((op1 == 3'b100 && op2 == 3'b101) || (op1 == 3'b101 && op2 == 3'b100)) begin
                    inject_hazard = (ic < 50) ? 100 : ($unsigned($random(current_seed)) % 100);
                    if (inject_hazard > 20) begin
                        for (k = 0; k < 8; k = k + 1) begin
                            ...
                            if (target_imm2 >= -64 && target_imm2 <= 63) begin
                                rs2 = k;
                                temp_imm2 = target_imm2; addr2_index = addr1_index; 
                            end
                        end
                    end
                end
            ...
            if (op1 == 3'b000 || op1 == 3'b001) prev_rd1 = rd1;
            else prev_rd1 = rt1;
        ...
        // 將該次 Testcase 的結果寫入檔案。若 RTL fail，留下來的就是這組死亡測資
        f1 = $fopen("../00_TESTBED/DRAM/inst_1_dram_file.dat", "w");
        if (f1 == 0) f1 = $fopen("inst_1_dram_file.dat", "w");
        ...
        if (f2 == 0) f2 = $fopen("inst_2_dram_file.dat", "w");
        ...
        if (fd == 0) fd = $fopen("data_dram_file.dat", "w");
...
// 取消註解可以開啟 AXI Snooper (馬拉松模式下可能會洗版)
/*
always @(posedge clk) begin
    ...
    begin
        while (!sim_halt_condition) begin
            ...
            if (stall_1 === 1'bx || stall_1 === 1'bz || stall_2 === 1'bx || stall_2 === 1'bz) begin
                $display("\n%s========================================================%s", `C_RED, `C_RST);
                $display("%s   [VIOLATION] Stall signal is Unknown (X) or High-Z!%s", `C_RED, `C_RST);
            ...
            if (core1_ic >= MAX_TRACE_LENGTH && !core1_halted) begin
                $display("\n%s[CRITICAL] Array Out of Bounds! Core 1 reached MAX_TRACE_LENGTH.%s", `C_RED, `C_RST);
                display_wrong; $finish;
            end
            if (core2_ic >= MAX_TRACE_LENGTH && !core2_halted) begin
                $display("\n%s[CRITICAL] Array Out of Bounds! Core 2 reached MAX_TRACE_LENGTH.%s", `C_RED, `C_RST);
                display_wrong; $finish;
            end

            if (core1_ic == next_dram_check_ic && core2_ic < next_dram_check_ic) begin
                $display("\n%s   [CRITICAL WARNING] Core 1 crossed 50-IC boundary (IC=%0d) while Core 2 is behind (IC=%0d).%s", `C_YLW, core1_ic, core2_ic, `C_RST);
            end
            if (core2_ic == next_dram_check_ic && core1_ic < next_dram_check_ic) begin
                $display("\n%s   [CRITICAL WARNING] Core 2 crossed 50-IC boundary (IC=%0d) while Core 1 is behind (IC=%0d).%s", `C_YLW, core2_ic, core1_ic, `C_RST);
            end
            ...
            if (stall_1 === 0) begin
                if (core1_halted) begin
                    $display("\n%s========================================================%s", `C_RED, `C_RST);
                    $display("%s   [VIOLATION] Core 1 executed an instruction AFTER reaching Halt Condition (PC=4095)!%s", `C_RED, `C_RST);
                    $display("%s========================================================%s\n", `C_RED, `C_RST); 
                    display_wrong; $finish;
                end
                ...
                if (core1_ic > core2_ic + 1) begin
                    $display("\n%s========================================================%s", `C_RED, `C_RST);
                    $display("%s   [VIOLATION] Sequence Violation! Core 1 is executing too fast.%s", `C_RED, `C_RST);
                    $display("%s   Core 1 IC: %0d | Core 2 IC: %0d%s", `C_RED, core1_ic, core2_ic, `C_RST);
                    $display("%s========================================================%s\n", `C_RED, `C_RST);
                    display_wrong; $finish;
                end
            end else begin
            ...
            if (stall_2 === 0) begin
                if (core2_halted) begin
                    $display("\n%s========================================================%s", `C_RED, `C_RST);
                    $display("%s   [VIOLATION] Core 2 executed an instruction AFTER reaching Halt Condition (PC=4095)!%s", `C_RED, `C_RST);
                    $display("%s========================================================%s\n", `C_RED, `C_RST); 
                    display_wrong; $finish;
                end
                ...
                if (core2_ic > core1_ic + 1) begin
                    $display("\n%s========================================================%s", `C_RED, `C_RST);
                    $display("%s   [VIOLATION] Sequence Violation! Core 2 is executing too fast.%s", `C_RED, `C_RST);
            ...
            // 50-IC Checkpoint
            if (core1_ic >= next_dram_check_ic && core2_ic >= next_dram_check_ic && !pending_dram_check) begin
                pending_dram_check = 1;
            end

            if (pending_dram_check && outstanding_aw_d == 0 && outstanding_w_d == 0 && out_r_d == 0 && !awvalid_s_inf_data && !wvalid_s_inf_data && !arvalid_s_inf_data) begin
            ...
            if ((!core1_halted && latency_1 > `MAX_WAIT_READY_CYCLE) || 
                ...
                $display("\n%s========================================================%s", `C_RED, `C_RST);
                $display("%s   [VIOLATION] Stall Timed Out! Exceeded %0d cycles!%s", `C_RED, `MAX_WAIT_READY_CYCLE, `C_RST);
                $display("%s========================================================%s\n", `C_RED, `C_RST); 
                display_wrong; $finish;
            end
        ...
        if (stall_1 !== 1 || stall_2 !== 1) begin
            $display("\n%s========================================================%s", `C_RED, `C_RST);
            $display("%s   [VIOLATION] stall_1 and stall_2 MUST remain HIGH after all instructions are completed!%s", `C_RED, `C_RST);
            $display("%s========================================================%s\n", `C_RED, `C_RST);
            display_wrong; $finish;
        end
    ...
    begin
        for (reg_idx = 0; reg_idx < 8; reg_idx = reg_idx + 1) begin
            if (golden_reg_core1[core1_ic][reg_idx] !== `REG_C1(reg_idx)) begin 
                $display("\n%s========================================================%s", `C_RED, `C_RST);
                $display("%s   [VIOLATION] Core 1 Register r%0d mismatch at IC: %0d!%s", `C_RED, reg_idx, core1_ic, `C_RST);
    ...
    begin
        for (reg_idx = 0; reg_idx < 8; reg_idx = reg_idx + 1) begin
            if (golden_reg_core2[core2_ic][reg_idx] !== `REG_C2(reg_idx)) begin 
                $display("\n%s========================================================%s", `C_RED, `C_RST);
                $display("%s   [VIOLATION] Core 2 Register r%0d mismatch at IC: %0d!%s", `C_RED, reg_idx, core2_ic, `C_RST);
                $display("%s--------------------------------------------------------%s", `C_RED, `C_RST);
    ...
    begin
        ...
        for (idx = 0; idx < 4096; idx = idx + 1) begin
            ...
            if (dram_snapshots[snapshot_idx][idx] !== rtl_dram_data) begin
                $display("\n%s========================================================%s", `C_RED, `C_RST);
                $display("%s   [VIOLATION] Data DRAM mismatch at 50-IC Checkpoint (IC=%0d)!%s", `C_RED, target_ic, `C_RST);
    ...
    begin
        for (idx = 0; idx < 4096; idx = idx + 1) begin
            ...
            if (golden_dram_data[idx] !== rtl_dram_data) begin
                $display("\n%s========================================================%s", `C_RED, `C_RST);
                $display("%s   [VIOLATION] Final Data DRAM mismatch at Word Index %0d (Byte Addr: %04x)!%s", `C_RED, idx, byte_addr, `C_RST);
                $display("%s--------------------------------------------------------%s", `C_RED, `C_RST);
