...
task init_and_reset_dut; begin
    // 1. 凍結 Clock，確保 Reset 傳遞時不會撞到任何 Clock edge
    force clk = 1'b0;  
    ...
    // 3. 停滯 3 個 Cycle，讓 rst_n = 0 走完所有 Buffer Tree
    #(`CYCLE_TIME * 3); 
    ...
    // 5. 再次停滯 3 個 Cycle，讓 rst_n = 1 走完所有 Buffer Tree
    #(`CYCLE_TIME * 3);
    
    // 6. 確保 Reset 已經完全穩定後，解除 Clock 的凍結
    release clk;
end endtask
